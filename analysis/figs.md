...
learn <- read.table("../stats/rwfit1000_l_params_tvalue_conj.csv", sep = ",", 
    header = TRUE)
...
random <- read.table("../stats/rwfit1000_r_params_tvalue_conj.csv", sep = ",", 
    header = TRUE)
...
levels(both$model) <- c("acc", "box_acc", "box_p", "box_rand", "box_rpe", "box_value", 
    "p", "rand", "rpe", "value")
...
levels(bothsig$model) <- c("acc", "box_acc", "box_p", "box_rand", "box_rpe", 
    "box_value", "p", "rand", "rpe", "value")
...
both$model <- revalue(both$model, c(box_acc = "acc", box_rpe = "RPE", box_value = "value", 
    box_p = "Pr", box_rand = "rand", rpe = "RPE", p = "Pr"))
...
bothsig$model <- revalue(bothsig$model, c(box_acc = "acc", box_rpe = "RPE", 
    box_value = "value", box_p = "Pr", box_rand = "rand", rpe = "RPE", p = "Pr"))
bothsig$bold <- factor(bothsig$bold, levels = c("acc", "Pr", "RPE", "value", 
    "rand"))
bothsig$model <- factor(bothsig$model, levels = c("acc", "Pr", "RPE", "value", 
    "rand"))
...
![plot of chunk f1andf2](figure/f1andf21.png) 

```r
print(f1.p2)
```

![plot of chunk f1andf2](figure/f1andf22.png) 

```r

# pdf(file='figs/model.pdf', width=5, height=8.5) multiplot(f1.p1, f1.p2,
# cols=1) dev.off()
...
![plot of chunk f1andf2](figure/f1andf23.png) 

```r
print(f2.p2)
```

![plot of chunk f1andf2](figure/f1andf24.png) 

```r

# pdf(file='figs/box_and_model.pdf', width=5, height=8.5) multiplot(f2.p1,
# f2.p2, cols=1) dev.off() pdf(file='figs/all_set.pdf', width=8, height=8.5)
# multiplot(f1.p1, f1.p2, f2.p1, f2.p2, cols=2) dev.off()
...
set <- read.table("../stats/rwset1000_l_params_tvalue_conj.csv", sep = ",", 
    header = TRUE)
set <- cbind(set$value, set$p, seperate_meta_set(as.character(set$path)))
colnames(set)
colnames(set) <- c("t", "p", "iter", "bold", "bold_alpha", "model", "model_alpha", 
    "design")
str(set)
...
print(f3.p1)
f3.p2 <- f3.sigpoints(setsig) + ggtitle("B")
print(f3.p2)
...
# p1 rpe tmps <- set[(set$bold == 'rpe'),] tmps <- tmps[grep('rpe',
# tmps$model),] str(tmps) setsig.rpe <- f3.sigfac(tmps) setsig.rpe$mean <-
# setsig.rpe$mean * 100 ## into percent str(setsig.rpe) f3.p1 <-
# f3.pdist(tmps, 'right'); print(f3.p1) f3.p3 <- f3.sigpoints(setsig.rpe);
# print(f3.p3) # p2 value tmps <- set[(set$bold == 'value'),] tmps <-
# tmps[grep('value', tmps$model),] setsig.val <- f3.sigfac(tmps)
# setsig.val$mean <- setsig.val$mean * 100 ## into percent f3.p2 <-
# f3.pdist(tmps, 'right'); print(f3.p2) f3.p4 <- f3.sigpoints(setsig.val);
# print(f3.p4)
```
